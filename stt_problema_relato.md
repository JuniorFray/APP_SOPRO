# Relato: qualidade do STT nativo (botão Home)

## Arquivo envolvido
lib/infrastructure/voice/voice_service.dart

## Pacote e versão
speech_to_text 6.6.2 (pubspec.lock)

## Problema
O botão Home usa o SpeechRecognizer nativo do Android (via speech_to_text)
para transcrever comandos de voz. A transcrição sai errada com frequência:
- Idioma trocado para inglês mesmo com localeId resolvido corretamente
  como pt_BR (confirmado por log stt_locale_resolved).
- Exemplos reais: "criar ambiente casa" -> "Em Chicago";
  "criar ambiente trabalho" -> "Creative creative audio";
  "comparar fone" -> "can you can you want me".
- O botão flutuante (overlay, pipeline nativo Kotlin separado, manda
  audio direto ao Gemini) funciona bem no MESMO aparelho, mesmo ambiente,
  mesmo momento -- ou seja, nao e o aparelho nem o ruido do ambiente.
- O Google Assistente nativo, no MESMO aparelho, reconhece bem em
  portugues -- ou seja, o motor de reconhecimento do Android em si nao
  e o problema, algo na nossa configuracao/uso do pacote esta.
- Encontrado um issue identico reportado no repositorio oficial do
  pacote (csdcorp/speech_to_text #569): localeId correto, mas a
  transcricao reverte pra ingles em algum ponto da sessao. Mesma versao
  do pacote (6.6.2), mesmo padrao de configuracao.

## O que precisamos alcançar
- Reconhecimento de voz em portugues confiavel e consistente no botao
  Home, na mesma qualidade que ja funciona no botao flutuante (Gemini) e
  no Google Assistente nativo do aparelho.
- Sem custo de API por comando, se possivel (motivo original da troca
  para STT nativo em vez de mandar audio ao Gemini).
- Sem reintroduzir os bugs ja corrigidos nesta sessao (eco do TTS,
  escuta sem o dedo pressionado, acumulo de restart-on-silence).

## Codigo atual (voice_service.dart)
```dart
import 'dart:async';
import 'dart:convert';
import 'dart:io';
